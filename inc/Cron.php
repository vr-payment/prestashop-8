...
class VRPaymentCron
{
    ...
    public static function cleanUpHangingCrons()
    {
        ...
        try {
            ...
            $sqlCleanUp = 'UPDATE ' . _DB_PREFIX_ . 'vrp_cron_job SET constraint_key = id_cron_job, state = "' .
                pSQL(VRPaymentCron::STATE_ERROR) . '" , date_finished = "' .
                pSQL($endTime->format('Y-m-d H:i:s')) . '", error_msg = "' .
                pSQL("Cron was not terminated correctly. Timeout exceeded.") . '" WHERE state =  "' .
                pSQL(VRPaymentCron::STATE_PROCESSING) . '" AND date_started < "' .
            ...
            PrestaShopLogger::addLog('Error updating hanging cron jobs.', 2, null, 'VRPayment');
        }
    }
    ...
    public static function insertNewPendingCron()
    {
        ...
        try {
            $sqlQuery = 'SELECT security_token FROM ' . _DB_PREFIX_ . 'vrp_cron_job WHERE state = "' .
                pSQL(VRPaymentCron::STATE_PENDING) . '"';
            ...
            $sqlInsert = 'INSERT INTO ' . _DB_PREFIX_ .
                'vrp_cron_job (constraint_key, state, security_token, date_scheduled) VALUES ( -1, "' .
                pSQL(self::STATE_PENDING) . '", "' . pSQL(VRPaymentHelper::generateUUID()) . '", "' .
    ...
    public static function getAllCronJobs()
    {
        $result = DB::getInstance()->query(
            'SELECT * FROM ' . _DB_PREFIX_ . 'vrp_cron_job ORDER BY id_cron_job DESC',
            false
        );
        $jobs = array();
    ...
    public static function getCurrentSecurityTokenForPendingCron()
    {
        $now = new DateTime();
        $sqlQuery = 'SELECT security_token FROM ' . _DB_PREFIX_ . 'vrp_cron_job WHERE state = "' .
            pSQL(VRPaymentCron::STATE_PENDING) . '" AND date_scheduled < "' .
    ...
    {
        $sqlQuery = 'DELETE FROM ' . _DB_PREFIX_ . 'vrp_cron_job WHERE (state = "' .
            pSQL(VRPaymentCron::STATE_SUCCESS) . '" OR state = "' .
            pSQL(VRPaymentCron::STATE_ERROR) . '") AND id_cron_job <= (
                SELECT id_cron_job
                FROM (
                  SELECT id_cron_job
                  FROM ' . _DB_PREFIX_ . 'vrp_cron_job
